...
TRACK REFERENCE vs TRACK CREATION:
- When "track" is used as a reference (e.g., "drum track", "bass track", "synth track"), extract the track name and do NOT create tracks
- ONLY create tracks when explicit creation words are used: "create", "add", "new", "make" + "track"
- "volume on synth track" means adjust volume on existing "synth" track, NOT create a track
...
Current session tracks:"""
    ...
    def _validate_and_enhance_command(self, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if action in ["add_audio_effect", "arm_track", "disarm_track"]:
